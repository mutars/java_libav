...
public class DefaultMediaWriter implements IMediaWriter {
    ...
    @Override
    public synchronized int addVideoStream(AVCodecID codecId, int width, int height) throws LibavException {
        if (isClosed())
    ...
    @Override
    public synchronized int addAudioStream(AVCodecID codecId, int sampleRate, AVSampleFormat sampleFormat, int channelCount) throws LibavException {
        if (isClosed())
            throw new IllegalStateException("the media stream has been closed");
